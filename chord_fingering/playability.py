...
def _same_fret_reachable(
    ...
    """同一品上相距较远的两根手指之间，不能夹着按在更高品的弦。

    手指是从指板上方压下去的：两根手指落在同一品的不同弦上时，若它们**相距较远**
    （中间隔着 2 根及以上弦）、且其中夹着按在更高品的弦，那根手指得从更高品的手指
    「上面」绕过去，实际弹不出来——真吉他手遇到这种情况只能改用食指横按压在下面。

    相邻或仅隔 1 根弦的同品两指可以并排落下：更高品的手指位于更靠琴桥一侧，不挡住
    这并排的两指。Bdim 的开放指法 ``x2343x`` 即此情形——D 弦 3 品与 B 弦 3 品之间
    只夹着 G 弦 4 品，用中指/无名指/小指按 3-4-3 品是标准按法。

    反例（这条规则加进来之前排在 F 前面的假指法）：``10321x``，6 弦 1 品和 2 弦 1 品
    要两根手指，中间夹着 4 弦 3 品和 3 弦 2 品（隔了 3 根弦）；而 F 的 ``133211`` 用食指
    横按解决同一问题，所以合法。同为「同品两指」的 G ``320003`` 也没问题——6 弦 3 品
    和 1 弦 3 品之间夹的全是空弦，手指从上方跨过去不碰任何东西。
    """
    for a, b in combinations(sorted(units, key=lambda u: u.lo), 2):
        if a.fret != b.fret or a.is_barre or b.is_barre:
            continue
        # 两根手指相隔的弦数（a.hi 与 b.lo 之间）。相邻或只隔 1 根弦时，更高品的手指
        # 落在更靠琴桥一侧，不挡住这并排的同品两指（Bdim 开放指法 x2343x 即此情形：
        # D 弦 3 品与 B 弦 3 品之间只夹 G 弦 4 品，中指/无名指/小指按 3-4-3 是标准按法）。
        # 只有两指相距较远、且中间夹着更高品弦时，外侧手指才需从更高品手指上方绕过而
        # 弹不出来（10321x 中 6 弦 1 品与 2 弦 1 品之间夹了 4 弦 3 品、3 弦 2 品）。
        gap = b.lo - a.hi - 1
        if gap < 2:
            continue
